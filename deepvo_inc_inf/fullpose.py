...
class FullPose7D(BaseExperiment):
    ...
    def __init__(self, in_folder, out_folder, args):
        ...
        _, (tmp, _, _, _) = next(enumerate(self.trainingset))
    ...
    def train(self):
        ...
        for i, (images, poses, _, label) in enumerate(self.trainingset):

            if prev_label is None:
                prev_label = label

            if prev_label != label:
                self.reset_hidden_state(state)
                print('Jumping from sequence {} to {}; Hidden state reset.'.format(prev_label, label))
            ...
            poses.squeeze_(0)
            if prev_pose is not None and prev_label == label:
                poses = torch.cat((prev_pose, poses), 0)
            prev_pose = poses[-1, :].unsqueeze(0)

            poses = self.convert_pose_to_incremental(poses)
            poses.unsqueeze_(0)

            # Prepend last image from previous sequence
            if prev_image is not None and prev_label == label:
            ...
            # Forward
            output, state = self.model.forward(input, state)
            ...
            self.optimizer.step()
            state = self.repack_hidden_state(state)
