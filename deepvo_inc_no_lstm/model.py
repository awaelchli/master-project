...
class FullPose7DModelNOLSTM(nn.Module):

    def __init__(self, input_size, hidden=500, nlayers=3, dropout=0):
        super(FullPose7DModelNOLSTM, self).__init__()
        ...
        self.fc = nn.Sequential(
            nn.Linear(fout[1] * fout[2] * fout[3], hidden),
            nn.ReLU(),
            nn.Linear(hidden, 6)
        )
    ...
    def forward(self, input, state=None):
        ...
        predictions = self.fc(pairs.view(n - 1, -1))

        return predictions, state

    def train(self, mode=True):
        super(FullPose7DModelNOLSTM, self).train(mode)
        #self.lstm.train(mode)

    def eval(self):
        super(FullPose7DModelNOLSTM, self).eval()
        #self.lstm.eval()

    def get_parameters(self):
        params = list(self.fc.parameters()) + list(self.layers.parameters())
        return params
