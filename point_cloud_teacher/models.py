...
class MultiClass1DTranslationModel(nn.Module):
    ...
    def __init__(self, hidden=500, nlayers=3, num_features=1, classes=10, dropout=0.0):
        ...
        self.lstm_input_size = self.feat_channels + 2
        ...
        self.eof_token = Variable(torch.zeros(1, self.lstm_input_size))
    ...
    def forward(self, input, prev_pose, state):
        # Input shape: [num_points, 2  + feat_channels]
        # Pose shape: [1] (class)
        num_points = input.size(0)
        feat_channels = self.feat_channels

        # Add batch dimension for LSTM
        lstm_input_tensor = input.unsqueeze(0)

        # LSTM input shape: [1, all_features, channels]
        init = (self.h0, self.c0) if not state else state
        ...
        eof_token = Variable(torch.zeros(1, 1, self.lstm_input_size)).cuda()
        eof_token[0, 0, 0] = prev_pose
