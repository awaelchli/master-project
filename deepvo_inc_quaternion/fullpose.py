...
from pose_evaluation import relative_euler_rotation_error, relative_quaternion_rotation_error, error_distribution, translation_error_per_meters, measure_distance_along_path, relative_rotation_error_per_meters_from_euler_pose
...
class FullPose7D(BaseExperiment):
    ...
    def train(self):
        ...
        for i, (images, poses, _) in enumerate(self.trainingset):
            ...
            poses.squeeze_(0)
            poses = self.convert_pose_to_incremental_quat(poses)
            poses.unsqueeze_(0)
            ...
            # Loss function
            loss, r_loss, t_loss = self.loss_function_q(output, target[:, 1:])
    ...
    def test(self, dataloader=None):
        ...
        last_frame_targets = []
        all_filenames = []
        all_outputs = torch.Tensor(len(dataloader), self.sequence_length - 1, 7)
        all_targets = torch.Tensor(len(dataloader), self.sequence_length - 1, 7)
        all_original_targets = torch.Tensor(len(dataloader), self.sequence_length, 7)

        self.model.eval()
        for i, (images, poses, filenames) in enumerate(dataloader):

            orig_poses = self.convert_eul_to_quat(poses.squeeze(0))
            ...
            loss, r_loss, t_loss = self.loss_function_q(output.unsqueeze(0), target[1:].unsqueeze(0))
        ...
        all_converted_outputs = torch.stack([self.convert_pose_to_global_quat(output) for output in all_outputs])
        all_original_targets = torch.stack([self.convert_quat_to_eul(t) for t in all_original_targets])
    ...
    def loss_function(self, output, target, average=True):
        # Dimensions: [batch, sequence_length, 7]
        bs = output.size(0)
        t1 = output[:, :, :3]
        t2 = target[:, :, :3]
        e1 = output[:, :, 3:]
    ...
    def convert_pose_to_global_quat(self, output):
        # Converts output of incremental poses to poses that are relative to the first frame
        # output: [sequence, 6]
        output = torch.cat((torch.zeros(1, 7), output), 0)
        output[0, 3] = 1
        matrices = [pose_transforms.quaternion_pose_vector_to_matrix(pose.view(1, -1)) for pose in output]
    ...
    def convert_eul_to_quat(self, poses):
        # poses: [sequence, 6]
        matrices = [pose_transforms.euler_pose_vector_to_matrix(pose.view(1, -1)) for pose in poses]
        poses = [pose_transforms.matrix_to_quaternion_pose_vector(m) for m in matrices]
        poses = torch.cat(tuple(poses), 0)
        return poses

    def convert_quat_to_eul(self, poses):
        # poses: [sequence, 7]
        matrices = [pose_transforms.quaternion_pose_vector_to_matrix(pose.view(1, -1)) for pose in poses]
        poses = [pose_transforms.matrix_to_euler_pose_vector(m) for m in matrices]
        poses = torch.cat(tuple(poses), 0)
        return poses

    def convert_pose_to_incremental_quat(self, poses):
        # Converts poses in global coordinates to incremental poses (in coordinate frame of previous image)
        # poses: [sequence, 7]
        matrices = [pose_transforms.euler_pose_vector_to_matrix(pose.view(1, -1)) for pose in poses]
        matrices = torch.stack(matrices)
    ...
    def loss_function_q(self, output, target):
        ...
        return self.beta * loss1 + loss2, loss1, loss2
